...
`stop_time` must be a nonnegative integer multiple of `time_step`; configuration creation rejects an unaligned stop instead of taking a short final step. For example, `time_step=100e-6, stop_time=250e-6` fails: use a 200/300 μs stop, or a 50 μs step. Pairwise 8 ULP tolerance handles rounding, so `time_step=0.1, stop_time=0.1+0.2` is valid and the declared stop remains the final timestamp. Explicit `insert` events can still create short intervals, so dynamic components must use `StampContext.time_step`. Times must be finite real scalars and the step must be positive; invalid types, integration methods and event policies report repair advice or supported choices.
...
`result.series("time")` is the authoritative time axis. The nominal `time_step` is a configuration value and does not replace the actual time differences when an event policy changes the spacing. `event_log` records topology changes and is serialized in JSON and NPZ. CSV remains a plain table and does not carry all metadata.
