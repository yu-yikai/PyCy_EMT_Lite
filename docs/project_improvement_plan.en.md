...
| C filter assemblies | Retain L/LC/LCL as optional RLC assemblies for studying connections and damping. Fixed silently omitted negative/NaN resistances; construction validates all electrical parameters with repair advice. LC/LCL pass independent ODE, KCL, discrete energy and convergence checks for both methods; documentation distinguishes physical damping from backward Euler numerical dissipation |
| C segmented lines/three-phase transformers | Retain optional linear models for spatial discretization and three-phase ratios/phase shifts. Segmented lines pass cascaded-Pi phasor, uniform-line spatial convergence and terminal-energy checks. All four Y/Δ connections pass independent analytical startup, line/winding current, energy and time-convergence checks. Zero-leakage Δ/Y is allowed; secondary-delta circulating-current diagnostics remain. Corrected flux integration that used backward Euler under trapezoidal |
...
The explicit requirement to retain synchronous machines (2026-09-12) supersedes the earlier removal direction. Both implementations, the example and exclusive tests have been restored and corrected. This batch passes **392 tests and 13 headless examples**, with no default `outputs/`; three saved example 10 figures were inspected. New regressions cover analytical dq ports, copper loss/air-gap power, classical R–L energy, independent continuous ODEs, first-order mechanical convergence and parameter diagnostics. All 55 local Markdown links and the diff check pass. Wheel/sdist contain 42/98 entries, include both machines and exclude caches. Source-external installed-wheel checks pass for location/version, machine exports, a resistor solve, classical power balance and example 10. No files, dependencies, top-level APIs or solver frameworks were added; the numerical kernel is unchanged. This batch and the preceding reduction were committed as `9a11a00`. These are local results, without new remote CI or independent-agent review evidence.

The filter batch (2026-09-12, based on `9a11a00`) passes **484 tests and 13 headless examples**, with no default `outputs/`. Both Chinese and English LC snippets run directly. Changes stay in six existing files; the implementation only adds parameter checks, with no new files, dependencies or public APIs. Physical comparisons and numerical-damping limits are described in the existing power-electronics document. All 55 local documentation links and the diff check pass. Wheel/sdist contain 42/98 entries, exclude caches and retain the machines. Source-external installation checks pass for location/version, a resistor solve, invalid filter diagnostics and analytical LC initial/first-step values. These are local results.

The line/transformer batch (2026-09-13, continuing the worktree above) passes **512 tests and 13 headless examples**, with no default `outputs/`. Chinese and English segmented-line and Δ/Y snippets all run directly. Incorrect rejection of zero-leakage Δ/Y and inconsistent trapezoidal flux/current were reproduced before repair; zero-leakage secondary delta still raises an error with parameter repair advice. New evidence covers segmented phasors/spatial convergence, end KCL/storage, analytical startup for all four connections, line/winding currents, power, flux and time convergence, including off-grid and endpoint events. This batch changes ten existing files, fourteen together with the filter batch, with no new files, dependencies, public APIs or solver frameworks. Both machines and example 10 remain. All 57 local documentation links pass. Wheel/sdist contain 42/98 entries without caches. Source-external installation checks pass for location/version, the 25 root exports, a resistor solve, zero-leakage Δ/Y ratio, linear flux, secondary-delta diagnostics, segmented initial terminal current and machine imports. The diff check passes. These are local results; the worktree remains uncommitted, with no new remote CI or independent-agent review evidence.

The historical baseline had 65 tests, 16 examples and 43 package Python files. These describe the audit starting point, not pending work. MIT, the 25-symbol root API and one CI job already exist. Initialization, explicit dynamic events, fixed-grid delay lines, the models above and three-phase short-circuit metrics now have persistent regressions. Remaining work consolidates documents and unvalidated scope. The lead agent implemented and checked this batch; no new independent agent review was performed, and earlier review is not evidence for this change.
...
|---|---|---|
| P1 B-08 validation boundaries | The stated basic RLC, 05/06 three-phase RL, Pi/segmented-line, linear single/three-phase transformer, PWM, machine and filter checks are complete. Explicitly lagged saturation still lacks independent saturation-curve and energy evidence | Validation is limited to the stated equations and test conditions; linear results do not establish saturation, shared-core coupling or broadband fault accuracy |
| P1 C remaining scope descriptions | Machines and example 10 remain; excluded models are removed. L/LC/LCL, segmented lines and three-phase connections remain as directly tested optional lessons. The saturation approximation is outside the validated teaching scope | Clarify boundaries and unvalidated items during document consolidation, without expanding frameworks to preserve names |
...
| Pi/segmented lines; 08 | Retain the lumped-line lesson, with segmented lines as an optional spatial comparison. Pi-line checks cover AC phasors, KCL, whole-transient energy and time convergence. Segmented checks cover cascaded phasors, end KCL, storage/terminal work with zero line resistance, and N=1/2/4/8 convergence toward a uniform-line reference. Existing current outputs remain series-branch values |
| Single/three-phase transformers; 09 | Retain linear models; three-phase connections compare voltage ratios and shifts. Single-phase loaded phasors, magnetizing DC, core loss/storage/convergence are checked. All four Y/Δ connections add analytical startup, line/winding currents, flux and energy/convergence. Zero-leakage Δ/Y is allowed; secondary delta still requires nonzero leakage. No saturation, hysteresis or shared-core-coupling claims |
...
| L/LC/LCL | Retain optional single-phase RLC connection assemblies using basic component states. Put the usage example in existing power-electronics documentation, without another course script. Checks cover zero/positive resistance, loaded LC and two-source LCL transients, port directions, KCL, energy and convergence; no closed-loop converter expansion |
