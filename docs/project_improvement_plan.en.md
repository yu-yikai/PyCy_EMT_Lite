...
This batch (2026-09-11), based on `d0f0612`, completes specific checks for examples 08/09/12: **337 tests and 17 headless examples passed without `outputs/`**. Five saved figures were visually checked. Wheel/sdist contain no caches; source-external installed-wheel checks passed for location/version, a resistor solve and these three examples. Changes stay in existing examples, tests and bilingual documentation, with no numerical-kernel changes or new files, dependencies or public APIs. These are local results, with no push or new remote CI run.

The historical baseline had 65 tests, 16 examples and 43 package Python files. These describe the audit starting point, not pending work. MIT, the 25-symbol root API and one CI job already exist. Initialization, explicit dynamic events, fixed-grid delay lines and the three models above now have persistent regressions. Three-phase short-circuit metrics and remaining model reduction are still open. The lead agent implemented and checked this batch; no new independent agent review was performed, and earlier review is not evidence for this change.
...
|---|---|---|
| P1 B-08 model validation | The stated basic RLC, Pi-line, linear single-phase transformer and PWM checks are complete. Example 06 still needs fixed pre/fault/post RMS and phase power; three-phase composites and advanced candidates cannot inherit single-phase evidence | Complete example 06 fixed-window assertions, then assess remaining candidates under section 4; runnable examples do not establish physical validation |
...
Initialization, explicit events, AC RLC, Bergeron boundaries and specific checks for examples 08/09/12 are complete. Next complete example 06 three-phase short-circuit pre/fault/post RMS, phase sequence and phase power, then reduce candidate models and duplicate documentation under section 4. Work in existing files: failing assertions first, then implementation, then another agent's review. Use Luna/Terra for implementation/routine execution and Sol for numerical design/independent review, without more agent layers.
