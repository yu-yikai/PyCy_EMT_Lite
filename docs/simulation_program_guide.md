...
7. 在 t=0 应用当刻事件，保持储能初值求一致代数量；后续正时长区间才使用积分模型。
8. 求解线性方程，得到节点电压和支路电流。
9. 更新电容、电感等动态元件的历史状态。
10. 记录当前时刻结果。
11. 默认插入事件时刻：先按旧网络积分到事件左侧，再按声明顺序应用同刻事件，固定储能状态求一次右侧代数量。写回历史后只记录一行右侧结果，并保留逐条事件日志。
...
`stop_time` 必须为 `time_step` 的非负整数倍；创建配置时就拒绝非整步终止时间，不生成短末步。
例如 `time_step=100e-6, stop_time=250e-6` 会报错，应将终止时间改为 200/300 μs，或将步长改为 50 μs。
采用成对 8 ULP 容差处理浮点舍入，`time_step=0.1, stop_time=0.1+0.2` 合法，保存的末点仍为声明的终止时间。
事件选择 `insert` 时仍可能形成短区间，因此动态元件必须使用 `StampContext.time_step`。
时间必须是有限实数，步长必须大于零；非法时间类型、积分方法及事件策略会报错并列出修复方法或合法选项。
...
`t=0` 不执行积分；`stop_time=0` 仍求解并保存一行一致初值。例如 1 V、1 kΩ、1 μF、
零初压 RC 的首行应为 `vC=0 V`、`iC=1 mA`。首个正时间区间从这组历史量出发。
电容与理想电压源并联时，两者初压必须相等；并联电容初压也必须一致，初始电流按电容分配。
浮置电感节点用 KCL 的一次导数补足电压约束。冲突、一次导数后仍欠定或支路电流不唯一的结构明确报错。

`VoltageSource(..., derivative=...)` 与 `CurrentSource(..., derivative=...)` 接受常数或时间函数，
分别表示解析 dv/dt（V/s）和 di/dt（A/s），只在一致求解确实需要源导数时读取并检查有限性。
常数源默认导数为零，`ThreePhaseSource` 使用正弦解析导数；普通满秩 RC/RL 的 callable 无需补导数。
例如 `VoltageSource("V", "n", "0", lambda t: 1 + 3*t, derivative=3.0)` 与
`Capacitor("C", "n", "0", 2.0, initial_voltage=1.0)` 的初始电流为 6 A。
仅提供 `value(t)` 无法保证精确求导，不使用数值差分兜底。示例 08 给出了并联电容约束下的正弦源写法；
...
Bergeron 的传播时延必须是基础步长的正整数倍，实际事件时间必须对齐固定网格；不支持的配置在首次求解前拒绝。
非对齐的事件设定时间可通过 `quantize_up` 延后至网格点。历史按整数步读取，负时间历史为零；
每个时刻保存一帧，事件使用右侧端口值。已验证无预响应、匹配负载到达幅值、开路/短路反射及浮点边界。
...
先求解，再更新这些历史/输出量，最后记录当前行。一致求解只更新代数量，不推进储能状态。
