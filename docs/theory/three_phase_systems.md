...
示例 05 每相负荷为 20 Ω 串联 50 mH，线路电阻为 0.5 Ω。50 Hz 时负荷感抗约 15.7 Ω，
稳态电流应按 `I = Vsource / (Rline + Rload + jωLload)` 计算，负荷端电压为
`Vload = I(Rload + jωLload)`。0.06–0.10 s 的两个周期用于 RMS 和三相 P/Q 统计；
电流相对本相电源电压滞后约 37.46°。三相电感电流从零启动，包括 t=0 电源电压非零的 B/C 相。
...
示例 06 每相线路为 0.8 Ω 串联 5 mH，负荷仍为 50 Ω，故障电阻为 0.1 Ω；
0.04 s 投入，0.08 s 清除。各阶段满足 `L di/dt + (Rline + Req)i = vsource`，
其中正常时 `Req = Rload`，故障时 `Req = Rload || Rfault`。电感电流连续，
母线电压 `vbus = Req i` 和故障支路电流可以在事件时跳变。

清除时线路电流转入电阻负荷，默认参数的母线绝对峰值约 7.79 kV，随后按约 98 μs 的
`L/(Rline+Rload)` 时间常数衰减。因此步长取 10 μs。这里没有寄生电容、避雷器和电弧，
该电压是理想教学电路的结果。线路电流、故障电流和母线电压分别绘图，避免把故障电流跳变误认为电感电流不连续。
测试使用分段正弦特解加指数暂态，核对零初值、投入/清除时电流、全波形和固定窗口指标。
...
原始采样区间做插值；示例 06/07 分别使用 0.01–0.03、0.05–0.07、0.09–0.11 s 的完整
50 Hz 周期。06 的故障窗口仍含衰减分量，各相 RMS 可以不同，不能当作纯稳态相量；
每相电阻负荷的平均有功为 `Vbus_rms²/Rload`。`peak_abs` 仍返回窗口内的采样峰值，允许单点窗口。
