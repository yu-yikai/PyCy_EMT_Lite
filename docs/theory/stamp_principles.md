...
上述积分 stamp 用于正时长区间。t=0 固定 vC/iL 求一致 iC/vL；显式事件先按旧网络积分到左侧，
再修改电导并固定储能状态求右侧值。更新历史后记录一行结果，避免事件反向改变前一区间。
...
当前对象式实现要求 `τ` 为基础步长的正整数倍，停止时间及实际事件时间对齐固定网格，
并在首次求解前检查。按整数步索引读取已求解的历史帧，负时间使用零波，缺失历史直接报错；
不进行时间插值或保持末帧兜底。每个时刻只保存一帧，显式事件以右侧端口值替换同刻左侧值。
...
各段在同一元件中依次装配，共用求解器，独立保存串联和电容历史状态。
`sending/receiving` 输出取首段/末段串联电流，`average` 取全部串联电流的算术平均，正方向均为发送端到接收端。
它们不含端口并联电容电流；输入总电流为 `Ifirst+C/(2N)·dVsend/dt`，输出总电流为
`Ilast-C/(2N)·dVrecv/dt`。端口功率应使用总电流。
...
`ThreePhaseTransformer` 由三个单相绕组相组成，支持 Y/Y、Y/Δ、Δ/Y、Δ/Δ。Y 接每相连接在 `bus:相别` 与中性点之间；Δ 接使用：
...
二次侧为 Δ 且漏阻抗为零时，绕组环流不唯一；需设置正的漏电阻或漏感。Δ/Y 允许零漏阻抗。
Δ 线电流是相邻绕组电流之差，例如 `Ia=Iab-Ica`；一次绕组总电流还包含励磁和铁损。
绕组比、线电压相移和已验证边界见[线路与变压器说明](advanced_line_transformer_models.md)。
...
求解后更新励磁电流、一次侧电压和磁链；梯形法先读取上一步电压，再写回当前电压。
线性 Lm 下满足 `Δψ=Lm·Δim`；t=0 和事件右侧一致求解不推进磁链。
饱和电感仍由上一步磁链选择，尚未完成独立饱和曲线和能量验证，不包含磁滞回线、剩磁和频率相关铁耗。
...
`machines/synchronous.py` 中的经典模型使用三相理想内电势源和定子 R–L 支路，
电磁功率按内部电势与支路电流的乘积求和，区别于机端有功。R–L 储能使用原有 companion stamp。

`machines/park_generator.py` 中的四阶模型使用两轴代数端口
`Vd=Ed'-Rs*Id+Xq'*Iq`、`Vq=Eq'-Rs*Iq-Xd'*Id`。
经 abc 变换得到随转角变化的三相耦合阻抗，三个支路行都写入相间系数；不再用单个 d 轴电感替代。
定子电流为代数量，事件可以使其跳变。气隙功率等于机端功率加定子铜损。

两种模型均在正区间 stamp 前使用左端反馈显式推进机电状态一次，之后求本时刻网络；
`update_state()` 只保存网络反馈及经典模型的定子历史，事件右侧不重复推进。
输出中的转角、内电势、电压和电流属于同一时刻。完整方程、单位、初值、模型范围及验证见
[线路、变压器与同步机说明](advanced_line_transformer_models.md)，避免在此重复控制方程。
