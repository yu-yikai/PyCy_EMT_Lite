...
各模型如何从物理关系推到离散方程、写入 MNA 并更新历史量，详见[元件推导](component_derivations.md)。该文覆盖基础元件、三相网络、线路、变压器、电机、滤波与控制，并附源码和测试入口；本文继续维护参数和验证范围。

## 1. 已验证范围

本地验证记录（2026-09-14，含三端 VSC-HVDC 综合示例）：681 项测试通过，14 个算例均在无界面模式运行，默认未生成输出文件。
综合示例的模式、运行结果及模型差异见[专项说明](three_terminal_vsc_hvdc.md)。
这些是本地证据，不是远端 CI 状态，也没有声称完成实验数据或商业 EMT 软件全模型对标。
...
| 三端两电平 VSC-HVDC 示例 18 | 单站开环/闭环、双站、三站 FAST 故障恢复、PLL/dq/P/Vdc/Vac、桥端口功率及 DC 离散能量 | 参考模型的教学重实现，含 RC 缓冲及控制调整；[系统测试](../tests/test_three_terminal_vsc_hvdc.py)、[步后回调与抽样](../tests/test_step_callback.py) |
| `ThreePhasePiLine`、`ThreePhaseParallelRLCLoad` | 初值/第一步、输出字段；并联负荷另有额定功率换算检查 | 局部检查，未建立完整独立三相 AC/故障验证；三相/线路测试 |
| 控制块与 SRF-PLL | 基础功能、采样时刻一致性、初始锁定保持、平衡输入的独立 ODE 与一阶收敛 | 功能及特定平衡工况检查；无闭环电网/弱网工程验证；[控制测试](../tests/test_controls.py) |
...
- `IdealSwitch`：由布尔值、数值 0/1 或返回这些值的时间函数控制导通状态，导通时写入 `1/closed_resistance`，关断时写入 `open_conductance`。
  前者电阻必须为有限正数（Ω），后者电导必须有限且非负（S），可为零。

常量门极在构造时检查，时间函数返回值在每次 stamp 时检查；NaN/Inf、字符串、数组及其他数值均报错，
不会先转换为布尔值。错误包含开关名、仿真时刻及 `closed` 值，失败采样不会改写开关状态。
...
示例 18 通过可选 `on_step` 在每个 EMT 求解点显式更新三个独立控制器，命令在下一网络步作用；详见[控制与参考映射](three_terminal_vsc_hvdc.md)。
控制块的数值参数、输入、采样间隔和重置值会检查类型与有限性；拒绝 NaN/Inf、布尔数值及字符串。
采样间隔和低通时间常数必须大于零；`SampleDelay.steps` 必须为非负整数。先检查再更新，非法输入或重置值不会污染历史状态。
...
`SRFPLL.step(voltages_abc, time_step)` 接受**当前时刻**的电压采样，`time_step` 为自上次采样至今的间隔。
先用上次角频率积分角度，再用当前电压和该角度更新 PI；新角频率用于下一积分区间：
...
`nominal_frequency` 必须为有限正实数；`minimum_frequency/maximum_frequency` 是**绝对角频率**上下限，
给定时必须有限且包含额定值，`None` 表示该侧不限幅。内部 PI 对 Δω 使用 `[ωmin−ωnom, ωmax−ωnom]`，
保持原有冻结式抗积分饱和。比如额定 50 Hz、范围 45–55 Hz，应分别传 `2π50`、`2π45`、`2π55`；
额定锁定时输出仍为 50 Hz。非法或不包含额定值的范围在构造时明确报错。
这修正了旧版本将两个参数解释为修正量的行为；旧脚本若填写了 Δω 上下限，迁移时分别加上 `nominal_frequency`。
返回 `PLLState` 的 d/q 电压使用返回的角度计算；角度、角频率和电压均属于当前采样行。
`time_step` 必须是有限正实数，非法间隔在状态改变前报错并说明修复方法；零时刻观察初值不应调用 `step`。

[示例 11](../examples/11_pll_dynamic_response.py) 是纯控制循环，结果方法标记为 `explicit_control`，没有 MNA 网络。
它先记录声明的 t=0 角度、额定角频率及其 d/q 投影，再从第一个正时刻推进；`stop_time=0` 只产生初值行。
时间参数复用 `SimulationConfig` 校验，非整步终点在循环前报错；这不改变上述控制离散关系。
默认使用 230 V RMS、50 Hz、30° 电源相位偏移、100 μs 采样和 1 s 时长。
注意 PLL 对齐的是电压空间矢量：对这里的正弦电源，目标角度为 `2πft+offset−π/2`。

现有回归验证不同正采样间隔下的额定初始锁定、返回角度与 d/q 的同刻关系、示例初值/终点及非法间隔不推进状态。
PLL 的三相电压采样必须恰有三个有限实数；非法参数、采样或重置角度明确报错，拒绝采样后可继续使用原状态。
另覆盖 45–55 Hz 限幅、单边限幅、持续饱和与解除、重置，以及频率计算溢出时不保留部分 PI 更新。
平衡 325 V 峰值、50 Hz 输入、100 μs、PI 增益 80/1000 的测试仍检查约 0.1 s 后
`|vq|<5 V`、角频率误差 `<2 rad/s`。
另以独立连续方程 `dθ/dt=ωnom+Kp e+xi`、`dxi/dt=Ki e` 核对正时间暂态，
其中 `e=sin(θgrid−θ)/max(|cos(θgrid−θ)|,|sin(θgrid−θ)|,1/Vpeak)`，不调用 PLL 的变换或状态更新。

该对照使用额定 50 Hz、实际 52 Hz、325 V 峰值、初始角 0.4 rad、电网空间矢量初始角 0.7 rad、
PI 增益 80/1000、零积分初值，DOP853 参考窗口为 0–0.2 s。
100/50/25 μs 在共同正时间点的最大角度误差约为 0.002471/0.001230/0.000614 rad，
角频率误差约为 0.2168/0.1079/0.0538 rad/s，呈一阶收敛。
声明初始频率保持额定值，首区间沿用它；连续方程的瞬时比例反馈频率不用于核对 t=0 的声明输出。
这些证据限于给定的平衡输入与离散关系及所列输入边界，不验证频率/相位突变性能、弱网稳定性、
负序解耦、陷波器、专用限幅恢复或闭环并网控制。

PWM 辅助函数 `triangular_carrier()` 生成 [-1,1] 三角载波，`sine_pwm_duty()` 根据调制比和电角度给出占空比，
`carrier_compare()` 输出 0/1 比较结果。PWM 函数在比较前拒绝非有限量及错误类型；载波频率须为正，调制比须在 [0,1] 内。
空间矢量调制、三电平调制和死区不是当前承诺的功能。
